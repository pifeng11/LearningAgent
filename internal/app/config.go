...
type Config struct {
	...
	OpenRouterMetadataEnabled bool
}
...
func LoadConfig() Config {
	return Config{
		...
		ModelDefaultModel:         envOrDefault("MODEL_DEFAULT_MODEL", envOrDefault("OPENROUTER_MODEL", envOrDefault("DEEPSEEK_MODEL", "deepseek-v4-flash"))),
		...
		OpenRouterReasoningModel:  envOrDefault("OPENROUTER_REASONING_MODEL", "openai/gpt-4o"),
		OpenRouterSiteURL:         os.Getenv("OPENROUTER_SITE_URL"),
		OpenRouterAppTitle:        envOrDefault("OPENROUTER_APP_TITLE", "LearningAgent"),
		OpenRouterMetadataEnabled: envBool("OPENROUTER_METADATA_ENABLED", true),
	}
}
